...
- **Secure hashing** — hashes sensitive values with a dedicated `PERSONA_HASH_KEY`, falling back to `APP_KEY` so the package works with zero configuration.
...
You may optionally set a dedicated hash key in `.env`. When unset, Persona falls back to `APP_KEY`:

```dotenv
PERSONA_HASH_KEY=your-unique-secret-key
```

The fallback to `APP_KEY` means the package boots on any command (including `vendor:publish`, `key:generate`, and `migrate`) even when `PERSONA_HASH_KEY` is absent.
